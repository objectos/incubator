...
public class ElementValueIfaceStep extends AbstractJavaFileStep {

  public ElementValueIfaceStep(Consumer<JavaFile> javaFileWriter) {
  ...
  public final void elementSpec(ElementSpec elementSpec) {
    generateJavaFile(
        SpiType.PACKAGE,
        ifaceCode(elementSpec)
    );
  }

  private InterfaceCode ifaceCode(ElementSpec elementSpec) {
    return _interface(
        HtmlBoot.GENERATED,
        _public(), elementSpec.classNameValue(),
        _extends(SpiType.Value)
    );
  }
