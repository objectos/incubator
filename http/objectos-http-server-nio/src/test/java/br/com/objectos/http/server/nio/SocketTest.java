...
public class SocketTest extends AbstractTest {
  ...
  public void parse() throws IOException {
    try (Channel channel = newChannel()) {
      runInThread(HelloWorld::get);
      SocketRequest res = channel.accept(ByteBuffer.allocate(1024)).parse();
      assertEquals(res.toString(), "ResolvedSocketRequest{method=GET, path=/hello-world.txt}");
    }
  }
